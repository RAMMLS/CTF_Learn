...
// Создание роторов для согласных (20-позиционные)
vector<int> createConsonantRotor(const string& rotor_str) {
    vector<int> rotor(20);
    int count = 0;
    for (char c : rotor_str) {
        if (consonants.find(c) != string::npos) {
            rotor[count] = consonants.find(c);
            count++;
            if (count == 20) break;
        }
    }
...
// Создание роторов для гласных (6-позиционные)
vector<int> createVowelRotor(const string& rotor_str) {
    vector<int> rotor(6);
    int count = 0;
    for (char c : rotor_str) {
        if (vowels.find(c) != string::npos) {
            rotor[count] = vowels.find(c);
            count++;
            if (count == 6) break;
        }
    }
...
bool loadAndPrepareRotors(const string& filename) {
    ...
    while (getline(file, line)) {
        if (line.length() == 26) {
            // Создаем роторы для согласных и гласных
            consonant_rotors.push_back(createConsonantRotor(line));
            vowel_rotors.push_back(createVowelRotor(line));
            loaded++;
        }
    }
...
int main() {
    ...
    int total_rotors = min(100, (int)consonant_rotors.size()); // Ограничиваем для теста
